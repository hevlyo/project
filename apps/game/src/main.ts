...
import {
	...
	resolveStatusChip,
	resolveStatusLine,
} from "./client/gameHud.js";
import {
	applyInputCommand,
	createInputState,
	resetInputState as resetInputKeys,
...
class GameApp {
	...
	bindWindowEvents() {
		...
		this.bindEvent(globalThis, "pointermove", (event) => {
			...
			this.scene.adjustFreeCameraLook(
				event.movementX || 0,
				event.movementY || 0,
			);
		});
	...
	tryStartDash() {
		...
		const dashUnlimitedActive = this.isDashUnlimitedActive(localPlayer, nowMs);

		if (!dashUnlimitedActive && nowMs < this.dashCooldownUntilMs) {
	...
	tryStartFireball() {
		...
		if (
			isTimedStateActive(localPlayer.attackCooldownUntil, this.getServerNowMs())
		) {
			this.ui.showToast("Fire ball ainda está em recarga.", "warning", 1200);
			return;
		}
	...
	handleStart() {
		...
		this.ui.setMenuStatus(
			"Chamando o servidor. Ele não quer te ver, mas deve atender.",
			"warm",
		);
		...
		if (!this.socket) {
			...
			this.ui.setMenuStatus(
				"Socket.IO não carregou. Sem WebSocket, sem humilhação. Problema seu.",
				"danger",
			);
			return;
		}
	...
	createSocket() {
		...
		this.socket.on("disconnect", (reason) => {
			...
			this.ui.showToast(
				`Conexão morreu: ${reason}. Igual seus sonhos.`,
				"warning",
				2400,
			);
		});
		...
		if (this.socket.io) {
			...
			this.socket.io.on("reconnect_failed", () => {
				...
				this.ui.setMenuStatus(
					"O servidor cansou de você. Fica assim ou tenta de novo.",
					"danger",
				);
	...
	makePlayerState(payload) {
		this.syncServerClock(payload?.serverTimeMs);
		this.syncServerClockFromCooldownPayload(payload);
		const nowMs = this.getServerNowMs();
		...
		return {
			...
			maxHealth: payload.maxHealth || 1,
			invulnerableUntil: this.normalizeTimedUntil(
				payload.invulnerableUntil,
				10_000,
				nowMs,
			),
			speedBoostUntil: this.normalizeTimedUntil(
				payload.speedBoostUntil,
				SETTINGS.speedBoostDurationMs * 2,
				nowMs,
			),
			dashCooldownUntil: this.normalizeTimedUntil(
				payload.dashCooldownUntil,
				SETTINGS.dashCooldownMs * 2,
				nowMs,
			),
			dashUnlimitedUntil: this.normalizeTimedUntil(
				payload.dashUnlimitedUntil,
				SETTINGS.infinityDashesDurationMs * 2,
				nowMs,
			),
			attackCooldownUntil: this.normalizeTimedUntil(
				payload.attackCooldownUntil,
				4000,
				nowMs,
			),
			position,
	...
	upsertPlayerFromServer(payload, { hardSync } = {}) {
		...
		player.health = payload.health || 0;
		player.maxHealth = payload.maxHealth || 1;
		player.invulnerableUntil = this.normalizeTimedUntil(
			payload.invulnerableUntil,
			10_000,
			nowMs,
		);
		player.speedBoostUntil = this.normalizeTimedUntil(
			payload.speedBoostUntil,
			SETTINGS.speedBoostDurationMs * 2,
			nowMs,
		);
		player.dashCooldownUntil = this.normalizeTimedUntil(
			payload.dashCooldownUntil,
			SETTINGS.dashCooldownMs * 2,
			nowMs,
		);
		player.dashUnlimitedUntil = this.normalizeTimedUntil(
			payload.dashUnlimitedUntil,
			SETTINGS.infinityDashesDurationMs * 2,
			nowMs,
		);
		player.attackCooldownUntil = this.normalizeTimedUntil(
			payload.attackCooldownUntil,
			4000,
			nowMs,
		);
	...
	syncProjectiles(payloads, { replaceAll } = {}) {
		if (replaceAll) {
			...
			for (const projectileId of this.projectiles.keys()) {
				if (!ids.has(projectileId)) {
					...
					this.scene.removeProjectile(projectileId);
				}
			}
		}
		...
		for (const payload of payloads) {
			...
			existing.radius = payload.radius;
			existing.damage = payload.damage;
			existing.color = payload.color;
			existing.createdAt = payload.createdAt;
			existing.expiresAt = payload.expiresAt;
			this.projectiles.set(payload.id, existing);
		}
	}

	handleProjectileSpawned(payload) {
		if (!payload?.id) {
			return;
		}
		...
		projectile.radius = payload.radius;
		projectile.damage = payload.damage;
		projectile.color = payload.color;
		projectile.createdAt = payload.createdAt;
		projectile.expiresAt = payload.expiresAt;
		this.projectiles.set(payload.id, projectile);
	}
	...
	handleFireballImpact(payload) {
		if (!payload?.projectileId) {
			return;
		}

		this.handleProjectileDestroyed(payload.projectileId);
		this.scene.spawnFireballImpact(
			payload.position,
			payload.color,
			this.simulationTimeMs,
			payload.wasFatal,
		...
		if (payload.playerId === this.localPlayerId) {
			this.scene.triggerPickupCameraBoost(
				Math.max(6, payload.damage || 0),
				this.players.get(this.localPlayerId)?.sizeMultiplier || 1,
			);
			if (payload.wasFatal) {
				this.ui.showToast("Você explodiu. Clássico.", "danger", 1800);
			}
		}
	}
	...
	handlePlayerState(payload) {
		...
		if (payload?.id === this.localPlayerId) {
			if (payload?.dashCooldownUntil) {
				this.dashCooldownUntilMs = this.normalizeTimedUntil(
					payload.dashCooldownUntil,
					SETTINGS.dashCooldownMs * 2,
				);
			}
	...
	updateRemotePlayers() {
		for (const player of this.players.values()) {
			...
			const delta = this.remoteDelta.subVectors(
	...
	updateLocalPlayer(player, { allowNetwork }) {
		...
		const nowMs = this.getServerNowMs();
		const boosted = isTimedStateActive(player.speedBoostUntil, nowMs);
		let speed = this.keys.sprint
		...
		const dashUnlimitedActive = this.isDashUnlimitedActive(player, nowMs);
		const dashReady = dashUnlimitedActive || nowMs >= this.dashCooldownUntilMs;
		if (this.pendingDash && dashReady) {
			...
			const nextDashCooldownUntil = nowMs + SETTINGS.dashCooldownMs;
			this.dashCooldownUntilMs = nextDashCooldownUntil;
			player.dashCooldownUntil = nextDashCooldownUntil;
	...
	updateHud({ force } = {}) {
		...
		const statusChip = resolveStatusChip(localPlayer, nowMs);
		const dashUnlimited = this.isDashUnlimitedActive(localPlayer, nowMs);
		...
		this.ui.updateHUD({
			...
			statusLine,
			statusChip,
			dashCooldownRatio: this.getDashCooldownRatio(nowMs),
			dashUnlimited,
			dashReady: dashUnlimited || nowMs >= this.dashCooldownUntilMs,
			dashRemainingMs: Math.max(0, this.dashCooldownUntilMs - nowMs),
		});
	}

	getDashCooldownRatio(nowMs = Date.now()) {
		const localPlayer = this.players.get(this.localPlayerId);
	...
	isDashUnlimitedActive(player, now = Date.now()) {
		return isTimedStateActive(player?.dashUnlimitedUntil, now);
	}

	getServerNowMs(localNow = Date.now()) {
		return localNow + this.serverClockOffsetMs;
	}

	syncServerClock(serverTimeMs, receivedAt = Date.now()) {
	...
	syncServerClockFromCooldownPayload(payload) {
		if (!payload || Number.isFinite(payload?.serverTimeMs)) {
			return;
		}
	...
	normalizeTimedUntil(untilMs, maxDurationMs, nowMs = this.getServerNowMs()) {
		const parsedUntil = Number(untilMs) || 0;
		if (!Number.isFinite(parsedUntil) || parsedUntil <= nowMs) {
			return 0;
		}

		const safeMaxDuration = Math.max(0, Number(maxDurationMs) || 0);
		const maxUntil = nowMs + safeMaxDuration;
		return Math.min(parsedUntil, maxUntil);
	}
	...
	getArenaObstacleCircles() {
		return [
			...this.getRingObstacleCircles(
				SETTINGS.arenaPostCount,
				SETTINGS.arenaPostRingScale,
				SETTINGS.arenaPostRadius,
			),
			...this.getRingObstacleCircles(
				SETTINGS.arenaMonolithCount,
				SETTINGS.arenaMonolithRingScale,
				SETTINGS.arenaMonolithRadius,
			),
			...this.getRingObstacleCircles(
				SETTINGS.arenaLanternCount,
				SETTINGS.arenaLanternRingScale,
				SETTINGS.arenaLanternRadius,
			),
			...this.getRingObstacleCircles(
				SETTINGS.arenaPlantCount,
				SETTINGS.arenaPlantRingScale,
				SETTINGS.arenaPlantRadius,
			),
		];
	}
	...
	resolvePostCollisions(previousPosition, player) {
		...
		for (let pass = 0; pass < 2; pass += 1) {
			...
			for (const obstaclePosition of obstacles) {
				if (
					this.resolveObstacleSlide(
						previousPosition,
						player,
						obstaclePosition,
						obstaclePosition.radius + SETTINGS.collisionPadding,
						skin,
					)
				) {
					corrected = true;
	...
	buildTextState() {
		...
		return {
			...
			coordinates:
				"origin=center, +x=right/east, +z=down/south from default menu view, +y=up",
