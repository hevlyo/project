...
export function createUIController(elements) {
	...
	function bindStart(handler) {
		...
		if (state.nicknameKeydownHandler) {
			elements.nicknameInput.removeEventListener(
				"keydown",
				state.nicknameKeydownHandler,
			);
		}
		...
		elements.nicknameInput.addEventListener(
			"keydown",
			state.nicknameKeydownHandler,
		);
	}
	...
	function bindMusicModeChange(handler) {
		...
		if (state.musicModeChangeHandler) {
			elements.musicModeSelect.removeEventListener(
				"change",
		...
		elements.musicModeSelect.addEventListener(
			"change",
			state.musicModeChangeHandler,
		);
	}
	...
	function bindMusicVolumeChange(handler) {
		...
		if (state.musicVolumeInputHandler) {
			elements.musicVolumeInput.removeEventListener(
				"input",
		...
		elements.musicVolumeInput.addEventListener(
			"input",
			state.musicVolumeInputHandler,
		);
	}
	...
	function setMusicMuted(isMuted) {
		...
		elements.musicMuteButton.setAttribute(
			"aria-pressed",
			muted ? "true" : "false",
		);
	...
	function bindMusicMuteToggle(handler) {
		...
		if (state.musicMuteClickHandler) {
			elements.musicMuteButton.removeEventListener(
				"click",
		...
		elements.musicMuteButton.addEventListener(
			"click",
			state.musicMuteClickHandler,
		);
	}
	...
	function updateHUD({
		...
		if (elements.dashLabel && elements.dashCooldownFill) {
			const safeRatio = Number.isFinite(dashCooldownRatio)
				...
				: 1;
			const safeRemainingMs = Math.max(0, Number(dashRemainingMs) || 0);
			const safeRemainingSeconds = Math.min(30, safeRemainingMs / 1000);
			const dashLabel = dashUnlimited
				? "Ilimitado!"
				: dashReady
					? "Pronto"
					: `${safeRemainingSeconds.toFixed(1)}s`;
